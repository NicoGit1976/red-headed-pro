 ...
 */
if ( isset( $_POST['rh_dest_save'] ) && check_admin_referer( 'rh_dest_save' ) && current_user_can( 'manage_woocommerce' ) ) {
    update_option( 'red_headed_default_email_to',        sanitize_text_field( $_POST['email_to']        ?? '' ) );
    update_option( 'red_headed_default_email_subject',   sanitize_text_field( $_POST['email_subject']   ?? '' ) );
    update_option( 'red_headed_default_email_from',      sanitize_email( $_POST['email_from']           ?? '' ) );
    update_option( 'red_headed_default_email_from_name', sanitize_text_field( $_POST['email_from_name'] ?? '' ) );
    update_option( 'red_headed_default_email_cc',        sanitize_text_field( $_POST['email_cc']        ?? '' ) );
    update_option( 'red_headed_default_email_bcc',       sanitize_text_field( $_POST['email_bcc']       ?? '' ) );
    update_option( 'red_headed_default_email_body',      wp_kses_post( $_POST['email_body']             ?? '' ) );
    update_option( 'red_headed_default_sftp_host',       sanitize_text_field( $_POST['sftp_host']       ?? '' ) );
    update_option( 'red_headed_default_sftp_port',       (int) ( $_POST['sftp_port'] ?? 22 ) );
    update_option( 'red_headed_default_sftp_user',       sanitize_text_field( $_POST['sftp_user']       ?? '' ) );
    update_option( 'red_headed_default_sftp_path',       sanitize_text_field( $_POST['sftp_path']       ?? '/' ) );
    if ( ! empty( $_POST['sftp_pass'] ) ) {
        update_option( 'red_headed_default_sftp_pass_enc', Red_Headed_Destination_Base::encrypt( wp_unslash( $_POST['sftp_pass'] ) ) );
    }
    update_option( 'red_headed_default_local_folder_path', sanitize_text_field( $_POST['local_folder_path'] ?? '' ) );
...
$local_dir = get_option( 'red_headed_default_local_folder_path', '' );
?>
<div class="pl-pane">
    ...
    <form method="post" class="pl-form">
        <?php wp_nonce_field( 'rh_dest_save' ); ?>
        ...
        <fieldset class="pl-card">
            ...
            <label class="pl-field">
                ...
                <input type="text" name="email_to" value="<?php echo esc_attr( get_option( 'red_headed_default_email_to', '' ) ); ?>" placeholder="<?php esc_attr_e( 'Comma-separated emails', 'red-headed-pro' ); ?>" />
            </label>
            <div class="pl-grid pl-grid-2">
                <label class="pl-field">
                    <span class="pl-field-lbl"><?php esc_html_e( 'Default from email', 'red-headed-pro' ); ?></span>
                    <input type="email" name="email_from" value="<?php echo esc_attr( get_option( 'red_headed_default_email_from', '' ) ); ?>" placeholder="<?php esc_attr_e( 'WordPress default', 'red-headed-pro' ); ?>" />
                </label>
                <label class="pl-field">
                    <span class="pl-field-lbl"><?php esc_html_e( 'Default from name', 'red-headed-pro' ); ?></span>
                    <input type="text" name="email_from_name" value="<?php echo esc_attr( get_option( 'red_headed_default_email_from_name', '' ) ); ?>" placeholder="<?php echo esc_attr( get_bloginfo( 'name' ) ); ?>" />
                </label>
            </div>
            <label class="pl-field">
                <span class="pl-field-lbl"><?php esc_html_e( 'Default subject', 'red-headed-pro' ); ?></span>
                <input type="text" name="email_subject" value="<?php echo esc_attr( get_option( 'red_headed_default_email_subject', '' ) ); ?>" placeholder="<?php esc_attr_e( 'New order received', 'red-headed-pro' ); ?>" />
            ...
            <div class="pl-grid pl-grid-2">
                <label class="pl-field">
                    ...
                    <input type="text" name="email_cc" value="<?php echo esc_attr( get_option( 'red_headed_default_email_cc', '' ) ); ?>" placeholder="<?php esc_attr_e( 'Comma-separated', 'red-headed-pro' ); ?>" />
                </label>
                <label class="pl-field">
                    <span class="pl-field-lbl"><?php esc_html_e( 'Default BCC', 'red-headed-pro' ); ?></span>
                    <input type="text" name="email_bcc" value="<?php echo esc_attr( get_option( 'red_headed_default_email_bcc', '' ) ); ?>" placeholder="<?php esc_attr_e( 'Comma-separated', 'red-headed-pro' ); ?>" />
                </label>
            </div>
            <label class="pl-field">
                <span class="pl-field-lbl"><?php esc_html_e( 'Default body', 'red-headed-pro' ); ?></span>
                <textarea name="email_body" rows="4"><?php echo esc_textarea( get_option( 'red_headed_default_email_body', "Hi,\n\nYour Red-Headed export is ready: {filename} ({records} orders).\n\n— The Lion Frog" ) ); ?></textarea>
            </label>
            <p class="pl-muted">
                <?php
                ...
                printf(
                    ...
                    Red_Headed_Soft_Lock::is_pro() ? esc_html__( 'Pro: unlimited', 'red-headed-pro' ) : esc_html__( 'Lite cap', 'red-headed-pro' )
                );
                ?>
            </p>
        </fieldset>
        ...
        <fieldset class="pl-card">
            ...
            <div class="pl-grid pl-grid-2">
                <label class="pl-field">
                    ...
                    <input type="text" name="sftp_host" value="<?php echo esc_attr( get_option( 'red_headed_default_sftp_host', '' ) ); ?>" placeholder="sftp.example.com" />
                </label>
                <label class="pl-field">
                    <span class="pl-field-lbl"><?php esc_html_e( 'Port', 'red-headed-pro' ); ?></span>
                    <input type="number" name="sftp_port" value="<?php echo (int) get_option( 'red_headed_default_sftp_port', 22 ); ?>" />
                </label>
                <label class="pl-field">
                    <span class="pl-field-lbl"><?php esc_html_e( 'User', 'red-headed-pro' ); ?></span>
                    <input type="text" name="sftp_user" value="<?php echo esc_attr( get_option( 'red_headed_default_sftp_user', '' ) ); ?>" />
                </label>
                <label class="pl-field">
                    <span class="pl-field-lbl"><?php esc_html_e( 'Password', 'red-headed-pro' ); ?></span>
                    <input type="password" name="sftp_pass" placeholder="<?php echo get_option( 'red_headed_default_sftp_pass_enc' ) ? '•••••• ' . esc_attr__( '(stored)', 'red-headed-pro' ) : ''; ?>" autocomplete="new-password" />
                </label>
                <label class="pl-field pl-grid-span-2">
                    <span class="pl-field-lbl"><?php esc_html_e( 'Remote path', 'red-headed-pro' ); ?></span>
                    <input type="text" name="sftp_path" value="<?php echo esc_attr( get_option( 'red_headed_default_sftp_path', '/' ) ); ?>" placeholder="/incoming/orders/" />
                </label>
            </div>
        ...
        <p>
            <button type="submit" name="rh_dest_save" class="pl-btn pl-btn-primary"><?php esc_html_e( '💾 Save defaults', 'red-headed-pro' ); ?></button>
        </p>
    </form>
</div>
