...
class Red_Headed_Destination_SFTP extends Red_Headed_Destination_Base {
    public static function ship( $file, $config ) {
        ...
        if ( class_exists( '\phpseclib3\Net\SFTP' ) ) {
            try {
                ...
                if ( ! $sftp->login( $user, $pass ) ) {
                    return new \WP_Error( 'sftp_auth', __( 'SFTP authentication failed.', 'red-headed-pro' ) );
                }
                if ( ! $sftp->put( $remote_path, $file, \phpseclib3\Net\SFTP::SOURCE_LOCAL_FILE ) ) {
                    return new \WP_Error( 'sftp_put', __( 'SFTP upload failed.', 'red-headed-pro' ) );
                }
                return true;
            } catch ( \Throwable $e ) {
        ...
        if ( function_exists( 'ssh2_connect' ) ) {
            ...
            if ( ! $conn ) return new \WP_Error( 'sftp_connect', __( 'SSH2 connect failed.', 'red-headed-pro' ) );
            if ( ! @ssh2_auth_password( $conn, $user, $pass ) ) return new \WP_Error( 'sftp_auth', __( 'SSH2 auth failed.', 'red-headed-pro' ) );
            $sftp = @ssh2_sftp( $conn );
            if ( ! $sftp ) return new \WP_Error( 'sftp_subsystem', __( 'SFTP subsystem failed.', 'red-headed-pro' ) );
            $stream = @fopen( "ssh2.sftp://{$sftp}{$remote_path}", 'w' );
            if ( ! $stream ) return new \WP_Error( 'sftp_open_remote', __( 'Cannot open remote path.', 'red-headed-pro' ) );
            $local = fopen( $file, 'r' );
            stream_copy_to_stream( $local, $stream );
            fclose( $local ); fclose( $stream );
            return true;
        }
        return new \WP_Error( 'sftp_no_lib', __( 'SFTP library missing. Install phpseclib3 (composer install --no-dev) or enable PHP SSH2 extension.', 'red-headed-pro' ) );
    }
}
