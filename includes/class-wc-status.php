...
/**
 * Red_Headed_WC_Status — registers an opt-in custom WooCommerce order status `wc-rh-exported`.
 *
 * Toggle: option `red_headed_register_wc_status_exported` (set in General settings).
 *
 ...
 */
class Red_Headed_WC_Status {

    const SLUG = 'wc-rh-exported';

    public static function init() {
        if ( ! get_option( 'red_headed_register_wc_status_exported', 0 ) ) return;
        if ( ! Red_Headed_Soft_Lock::is_available( 'wc_status_exported' ) ) return;
